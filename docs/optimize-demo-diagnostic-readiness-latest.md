...
| --- | --- | --- | --- | --- | --- |
| cop_closure_gap | review | COP 反算闭合偏差 | -88.8% | 冷量表 / 总功率表 / 负荷输入 | 只提示冷量/功率口径需核对，不自动修正 COP。 |

### 交叉校验

| key | 状态 | 数值 | 证据 | 复核对象 |
| --- | --- | --- | --- | --- |
| power_meter_closure | normal | 0% | 分项合计 1932.9 kW / 总功率 1932.9 kW | 冷站总电表、主机/泵/塔分项电表和计量口径 |
| cop_closure | review | -88.8% | 反算 COP 0.74 / 现态 COP 6.6 | 冷量表、总功率表和负荷输入口径 |
| wet_bulb_physical_boundary | normal | 2.9℃ | Tcws 28.7℃ / 湿球 25.8℃ | 湿球温度、冷却塔出水温度和安装位置 |
| trend_steady_state | normal | 11.4% | 冷站总功率 稳态窗口样本 10 个，波动 11.4%。 | 同负荷/相近湿球历史窗口 |
...
| branch_pressure_spread | watch | 支路压差离散 | 11.5kPa | 远近端支路压差、总管压差和压差控制点 | 只提示远近端压差分布需复核，不自动改压差设定。 |
...
| key | 优先级 | 任务 | 触发原因 | 边界 |
| --- | --- | --- | --- | --- |
| branch-hydraulic-history-window | P0 | 补支路水力历史趋势 | 实时支路流量最大/最小比 38.1。；实时支路压力差范围 11.5 kPa。 | 不把单次实时差异外推为全天失衡，不直接判定末端阀门故障。 |
...
| key | 状态 | 指标 | 数值 | 复核对象 | 边界 |
| --- | --- | --- | --- | --- | --- |
| chilled_delta_t_sawtooth | active | 冷冻水温差锯齿波 | 3次 | 冷冻水温差、冷冻泵频率和供水温控制回路 | 只提示温差锯齿波风险，不自动改 PID、不自动降泵。 |
| cooling_delta_t_sawtooth | normal | 冷却水温差锯齿波 | 1次 | 冷却水温差、冷却泵和冷却塔控制回路 | 只提示冷却侧震荡风险，不自动改塔风机或冷却泵控制参数。 |
| station_power_hunting | active | 冷站总功率 hunting | 31次 | 冷站功率 hunting 与设备状态切换 | 只提示功率 hunting 风险，不自动切换设备组合。 |
