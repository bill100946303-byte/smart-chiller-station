...
- 控制写入副作用: 无，当前仅生成执行包
- 生成时间: 2026-06-20T04:31:50.209Z

## 执行命令

```bash
FCU_CANARY_DEVICE_CODE=WSJ01 FCU_CANARY_COMMAND_KIND=setpoint FCU_SMALL_BATCH_CONFIRM=I_UNDERSTAND_REAL_BA_WRITE npm --prefix apps/chiller-bff run execute:fcu-canary-dispatch
```
...
| Canary 队列 | 通过 | target=WSJ01, first=WSJ01 devices=2 | npm --prefix apps/chiller-bff run build:fcu-canary-queue |
...
- WSJ01 单台真实写入执行报告 mode=confirmed_canary_dispatch
