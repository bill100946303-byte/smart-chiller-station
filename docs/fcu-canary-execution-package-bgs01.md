...
| Canary 队列 | 通过 | target=BGS01, first=BGS01 devices=2 | npm --prefix apps/chiller-bff run build:fcu-canary-queue |
...
- BGS01 单台真实写入执行报告 mode=confirmed_canary_dispatch
