...
start_bff() {
  ...
  start_launch_job \
    ...
    "$BFF_LOG" \
    /bin/zsh -lc \
    "cd \"$BFF_DIR\" && export BFF_PORT=\"$BFF_PORT\" LEGACY_BASE_URL=\"$LEGACY_BASE_URL\" REALTIME_PARAMS_BASE_URL=\"$REALTIME_PARAMS_BASE_URL\" REALTIME_PARAMS_TIMEOUT_MS=\"$REALTIME_PARAMS_TIMEOUT_MS\" APP_MODE=\"$APP_MODE\" APP_MODE_LABEL=\"$APP_MODE_LABEL\" READ_ONLY_MODE=\"$READ_ONLY_MODE\" && exec npm run dev"
