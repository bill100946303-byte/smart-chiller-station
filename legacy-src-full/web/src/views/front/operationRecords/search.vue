<template>
  <div class="operation-record-search">
    ...
    >
      <el-form-item :rules="[{ required: true, message: $t('prompt.pleaseSelect')+$t('logrizi.time') }]"
                    label="时间段选择"
      ...
      <el-form-item :rules="[{ required: true, message: '设备不能为空' }]" :label="$t('alertrun_realtime.deviceType')" prop="drTypeId">
        <el-cascader
   ...
   /* formInline: {
      handler(val) {
        /!* if (val["drId"]) {
           ...
           // this.$emit("drIdchange", val);
         }*!/
        // if (val["drTypeId"]) {
        // this.$emit("drIdchange", val);
        // console.log('drTypeId', val.drId)
        this.deviceName()
        // }
      },
      deep: true,
